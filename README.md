...
Importing what you already have: `node bin/hindsight-backfill.mjs --dry-run` estimates the cost of sending the curated memory files and recent Claude Code / Codex transcripts. Without `--dry-run` it sends them oldest to newest with curated files last, stops before `--max-usd` (default 40), skips unchanged documents, and aborts unless a canary proves the server redacts secrets. Extraction runs on your Hindsight server's model, so it costs whatever that server's provider charges.
